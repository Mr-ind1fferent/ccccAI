...
def predict_video(detector, camera_id):
    ...
    index = 1
    temp_dir = 'output/detect/' + video_name.split('.')[0] + '/temp/'
    if not os.path.exists(temp_dir):
        os.makedirs(temp_dir)
    print('temp_ready')

    with open(os.path.join(
            os.path.join(FLAGS.output_dir, str.split(str.split(FLAGS.video_file, '/')[-1], '.')[-2]) + '.txt',
            ), 'a') as f:
        while True:
            ...
            out_path = os.path.join(temp_dir, 'temp_'+str(index)+'.jpg')
            ...
            if 'boxes' in results and len(results['boxes']) > 0:
                for dt in results['boxes']:
                    ...
                    if score > FLAGS.threshold:
                        f.writelines(
                            str(index - 1) + ',-1,' + str(xmin) + ',' + str(ymin) + ',' + str(
                                xmax - xmin) + ',' + str(
                                ymax - ymin) + ',1' + '\n')

        print('INFO:\ttxt file save in',
              os.path.join(os.path.join(FLAGS.output_dir, os.path.split(FLAGS.video_file)[-2]), '.txt'))
